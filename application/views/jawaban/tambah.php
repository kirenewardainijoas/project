				...
				<div class="row">
					<div class="col">
						<div class="card shadow">
							...
							<div class="card-body">
								<form action="<?= base_url('jawaban/proses_tambah') ?>" id="form-tambah" method="POST">
									<h5>Data Customer</h5>
									<hr>
									<div class="form-row">
										<div class="form-group col-2">
											<label>ID Jawaban</label>
											<input type="text" name="id_jawaban" value="<?= $this->session->login['id_jawaban'] ?>" readonly class="form-control">
										</div>
										<div class="form-group col-3">
											<label>ID Pertanyaan</label>
											<input type="text" name="id_pertanyaan" value="<?= $this->session->login['id_pertanyaan'] ?>" readonly class="form-control">
										</div>
										<div class="form-group col-3">
											<label>Jawaban</label>
											<input type="text" name="jawaban" value="<?= $this->session->login['jawaban'] ?>" readonly class="form-control">
										</div>
									...
									<div class="form-row">
										<div class="form-group col-3">
											<label for="id_pertanyaan">Soal</label>
											<select name="soal" id="soal" class="form-control">
												<option value="">Pilih Pertanyaan</option>
												<?php foreach ($all_pertanyaan as $pertanyaan): ?>
													<option value="<?= $pertanyaan->soal ?>"><?= $pertanyaan->soal ?></option>
												<?php endforeach ?>
											</select>
										</div>
										<div class="form-group col-2">
											<label>ID Pertanyaan</label>
											<input type="text" name="id_pertanyaan" value="" readonly class="form-control">
										</div>
										...
										<table class="table table-bordered" id="keranjang">
											<thead>
												<tr>
													<td width="35%">ID Survei</td>
													<td width="15%">ID Customer</td>
													<td width="15%">ID Pertanyaan</td>
													<td width="10%">ID Jawaban</td>
